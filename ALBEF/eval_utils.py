...
def compute_metrics_from_ranked_productIds(qids,ranked_products,data_file,data_labels_file):
    q_rels={}
    ...
    q_preds = {str(qid):{str(product_id):len(ranked_products[qid_index])-product_index for product_index,product_id in enumerate(ranked_products[qid_index])} for qid_index,qid in enumerate(qids)}
